...
3. Note: it only work on Michigan ticket
'''
...
# Change below if needed for different query
        ...
        unknown_query = f"{arcpy.AddFieldDelimiters(layer, placement_col_name)} <> 'Unknown'"
        iru_query = f"{arcpy.AddFieldDelimiters(layer, owner_col_name)} <> 'IRU'"
        combined_query = f"{aerial_query} AND {unknown_query} AND {iru_query}"
...
def go_to_coordinate_and_capture_screenshots(map_name, longitude, latitude, req_name, ticket, spatial_reference=4326,
    ...
    try:
        ...
        for scale in scales:
            ...
            print(
                f"Zoomed to coordinate ({longitude}, {latitude}) with spatial reference {spatial_reference} "
    ...
    try:
        ...
        for filename in os.listdir(output_directory):
            if filename.endswith(".png"):
                ...
                img = image.load_img(img_path, target_size=(480, 480)) #Change this according to the model: 480,720,1080
