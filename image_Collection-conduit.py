...
---This code collect Conduit features images. It is designed to collect data for machine learning.
...
-Warning: Based on previous experience, the range of start and end index tend to be within 700-800 images per session,
            try to keep the range within that.
...
def show_max_features(app, max_features):
    QMessageBox.information(None, "Max Features", f"The maximum number of features is {max_features}.")
...
def get_start_end_indices(app):
    ...
    #Caution: If you set end to 100, it will take screenshot_99 then stop.
    end_index, ok2 = QInputDialog.getInt(None, "Input Dialog", "Enter the end index (x+1):", 100, 1)
    if not ok2:
        return None, None
...
def main():
    map_name = "Map"
    layer_name = "conduit_20240611"

    app = QApplication([])

    # Get feature count after applying filter
    max_features = get_feature_count(layer_name)
    ...
    map_view = maps[0]  # Assuming you're using the first map; adjust as needed
