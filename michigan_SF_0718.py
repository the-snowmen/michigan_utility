...
-Update 08/06/2024: michigan_SF.py has experience performance issues, michigan_SF_0718.py are working properly
...
from tensorflow.keras.models import load_model                                          #pip install tensorflow (ignore)
import numpy as np                                                                      #pip install numpy
import tensorflow as tf                                                                 #pip install tensorflow (ignore)

'''
TO-DO: Update the model address to the one that is on the local machine
Note: This will take a while to load the first time
Warning: for the file path, add r to the front, it means raw file path. ex. r"C:Users\Ever\..."
'''

# TO-DO: Update the model address to the one that is on the local machine
model = load_model(r"...\MikeDHuang_2024\Machine Learning\model\T106_v1.keras")
...
'''
1. This function extract coordinate, address, caller email from the Salesforce ticket
2. It look for a keywork (ex. LATITUDE=), and extract the text after it.
2. Note: it only work on Michigan ticket
'''
...
def regex_process(content):
    try:
        ...
        return latitude_match, longitude_match, address_match, city_match, state_match, email_match, name_match, req_name_match, ticket_match
    except Exception as e:
        print(f"Error in regex processing: {e}")
        return None, None, None, None, None, None, None, None, None

'''
1. This function uses the Definition Query on ArcGIS pro, basically a SQL code
2. Have to set up individual query, and combined them into one conintuour query.= (only one Def Query acan be active!)
3. '=' = is, '<>' = is not
'''
def filtered(layer_name, placement_col_name, owner_col_name):
    try:
        ...
        # Change below if needed for different query
        aerial_query = f"{arcpy.AddFieldDelimiters(layer, placement_col_name)} = 'Underground'"
...
def go_to_coordinate_and_capture_screenshots(map_name, longitude, latitude, req_name, ticket, spatial_reference=4326,
                                             scales=[1000, 2500, 5000, 10000, 20000, 30000], output_base_directory=None,
    ...
    try:
        ...
        for scale in scales:
            ...
            print(
                f"Zoomed to coordinate ({longitude}, {latitude}) with spatial reference {spatial_reference} on map '{map_name}' at scale {scale}")
            if save_screenshots:
        ...
        # Activate the classify model function
        classify_images_in_directory(output_directory)
    except Exception as e:
        print(f"Error in go_to_coordinate_and_capture_screenshots: {e}")

'''
This add a red border to the screenshot(s) when the model detects underground cable within.
'''
    ...
    try:
        ...
        for filename in os.listdir(output_directory):
            if filename.endswith(".png"):
                ...
                # Change this according to the model: 480,720,1080
...
def main():
    ...
    while True:
        ...
        latitude_match, longitude_match, address_match, city_match, state_match, email_match, name_match, req_name_match, ticket_match = regex_process(
            content)
        ...
        go_to_coordinate_and_capture_screenshots(map_name, longitude, latitude, req_name, ticket,
                                                 ...
                                                 save_screenshots=True)
        '''
        This creates an CSV file that saves the extracted information, destination are in the same folder user has selected.
        '''
